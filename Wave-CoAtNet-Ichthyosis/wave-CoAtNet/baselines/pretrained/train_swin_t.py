"""
Swin-T Fine-tuned Baseline
============================
Fine-tunes Swin Transformer Tiny (ImageNet pretrained via timm) on the ichthyosis dataset.
Uses layer-wise learning rate decay and label smoothing.

Usage:
    python baselines/train_swin_pretrained.py
"""
...
def plot_curves(history, prefix="swin_pretrained"):
    for metric in ['loss', 'acc']:
        ...
        plt.title(f'Swin-T (Pretrained) — {metric.capitalize()}')
...
def main():
    ...
    # Swin-T pretrained on ImageNet-1k
    model = timm.create_model(
        'swin_tiny_patch4_window7_224',
        pretrained=True,
        num_classes=num_classes
    ).to(DEVICE)
    print(f"Model: swin_tiny_patch4_window7_224 (pretrained=True)")
    ...
    for epoch in range(EPOCHS):
        ...
        if vl_acc > best_val_acc:
            best_val_acc = vl_acc
            torch.save(model.state_dict(), 'best_swin_pretrained.pth')
            print(f"  ✓ Best model saved (Val Acc = {best_val_acc:.4f})")

    print("\n--- Final Evaluation ---")
    model.load_state_dict(torch.load('best_swin_pretrained.pth', weights_only=True))
    _, final_acc, y_true, y_pred = evaluate(model, test_loader, criterion, "Final Test")
    print(f"Test Accuracy: {final_acc*100:.2f}%")
    print(classification_report(y_true, y_pred, target_names=class_names, digits=4))

    np.save('swin_pretrained_y_true.npy', y_true)
    np.save('swin_pretrained_y_pred.npy', y_pred)

    cm = confusion_matrix(y_true, y_pred)
    plt.figure(figsize=(12, 10))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=class_names, yticklabels=class_names, annot_kws={"size": 12})
    plt.title('Confusion Matrix — Swin-T (Pretrained)', fontsize=14, fontweight='bold')
    plt.xlabel('Predicted', fontsize=13); plt.ylabel('True', fontsize=13)
    plt.tight_layout()
    plt.savefig('confusion_matrix_swin_pretrained.png', dpi=300)
    plt.close()
